...
async function validate(connection, document, settings) {
    ...
    connection.sendDiagnostics({
        ...
        diagnostics: result.warnings.map((warning) => {
            ...
            const endPosition = typeof warning.endLine === "number" &&
                typeof warning.endColumn === "number"
                ? node_1.Position.create(warning.endLine - 1, warning.endColumn - 1)
                : position;
            return {
                code: warning.rule,
                message: warning.text,
                range: node_1.Range.create(position, endPosition),
